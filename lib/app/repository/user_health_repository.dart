...
class UserHealthRepository {
  ...
  Map<HealthDataType, List<Log>> _parseUserHealthData(List<HealthDataPoint> data) {
    final Map<HealthDataType, List<Log>> healthDataMap = {
  ...
  Future<Map<HealthDataType, List<Log>>> getUserStatisticsSnapshot({required Duration fetchDate}) async {
    ...
    if (hasPermissions == null || !hasPermissions) {
      final bool requested = await health.requestAuthorization(snapshotHealthDataTypes, permissions: permissions);
      if (!requested) {
    ...
    final Map<HealthDataType, List<Log>> healthDataMap = await isolateParseUserHealthData(data: healthData);
